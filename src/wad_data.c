...
wad_data init_wad_data(const char *path) {
  ...
  wd.nodes = get_nodes_from_lump(file, wd.directory, wd.map_index + NODES, 28,
                                 0, wd.len_nodes);
  wd.subsectors = get_subsectors_from_lump(
      file, wd.directory, wd.map_index + SSECTORS, 4, 0, wd.len_subsectors);
  wd.segments = get_segments_from_lump(file, wd.directory, wd.map_index + SEGS,
                                       12, 0, wd.len_segments);
