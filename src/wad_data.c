...
wad_data init_wad_data(const char* path){
    ...
    wd.len_segments = wd.directory[wd.map_index + SEGS].lump_size / 12; // 12 = number of bytes per seg
    ...
    wd.segments = get_segments_from_lump(file,wd.directory,wd.map_index + SEGS,12,0,wd.len_segments);
